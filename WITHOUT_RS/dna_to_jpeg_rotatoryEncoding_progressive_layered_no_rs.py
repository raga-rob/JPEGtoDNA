"""
dna_to_jpeg_rotatoryEncoding_progressive_layered_no_rs.py
...
def decode_jpeg_from_fragments(fragments_bytes, map_period, map_seed_base, whiten_flag, block_size, jpeg_len):
    """
    Decode JPEG from fragment bytes WITHOUT Reed-Solomon error correction.
    ...
    for list_index, frag in enumerate(fragments_bytes):
        ...
        # Parse block units (NO RS decoding - just extract raw data)
        cursor = 0
        for _ in range(num_blocks_in_fragment):
            if cursor + BLK_STRUCT.size > len(frag_body):
            ...
            # Calculate actual block size (might be smaller for last block)
            remaining_in_fragment = len(frag_body) - cursor
            actual_block_size = min(block_size, remaining_in_fragment)
            
            if cursor + actual_block_size > len(frag_body):
                break
            block_data = frag_body[cursor:cursor+actual_block_size]
            cursor += actual_block_size
            block_units.append((jpeg_offset, blk_index, block_data))

    # Assemble JPEG (NO RS decoding - just place blocks at their offsets)
    output = [0] * jpeg_len
    block_units.sort(key=lambda t: t[0])

    for jpeg_offset, blk_index, block_data in block_units:
        # No RS decoding - just place the raw data
        end = min(jpeg_offset + len(block_data), jpeg_len)
        if end > jpeg_offset:
            output[jpeg_offset:end] = list(block_data[:end - jpeg_offset])
...
def decode_jpeg_from_fasta(fasta_path, out_path, verify_sha256=True):
        ...
        for map_seed_base in range(4):
            for whiten_flag in [0, 1]:
                try:
                    ...
                    magic, version, block_size, reserved1, jpeg_len, jpeg_sha256, num_fragments_hdr, fragment_len_bases, map_period_hdr, map_seed_base_hdr, whiten_flag_hdr, reserved2 = header_tuple
    ...
    if found_header is None:
        raise ValueError("Failed to locate valid header; ensure input matches encoder v6 (no RS)")

    # Parse the found header
    magic, version, block_size, reserved1, jpeg_len, jpeg_sha256, num_fragments_hdr, fragment_len_bases, map_period, map_seed_base, whiten_flag, reserved2 = found_header
    ...
    # 4) Decode with all fragments (NO RS decoding)
    output_bytes = decode_jpeg_from_fragments(
        fragments_bytes, map_period, map_seed_base, whiten_flag, block_size, jpeg_len
    )
...
def main():
    parser = argparse.ArgumentParser(description="Decode DNA FASTA back into the original JPEG (v6, no RS) with progressive/layered decoding. Each strand in the FASTA is automatically decoded as a separate layer.")
